...
//void insertarAlInicio(struct Lista & tad, Pedido elemento) {
//
...
void insertarAlFinal(struct Lista & tad, Pedido elemento) {
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.id<< "("<<recorrido->elemento.tiempoEstViaje+recorrido->elemento.tiempoPreparacion<<")";
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
