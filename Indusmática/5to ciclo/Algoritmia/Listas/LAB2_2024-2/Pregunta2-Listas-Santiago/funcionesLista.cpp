...
void insertarAlInicio(struct Lista & tad, Pedido pedido) {
    ...
    struct Nodo * nuevoNodo = crearNodo(pedido, tad.cabeza);
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->pedido.id << " " << 
                    recorrido->pedido.tiempoEstimadoViaje + recorrido->pedido.tiempoPreparacion;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
