...
//        insertarAlInicio(tad, el);
//    }
//}

/*inserta un nodo en orden*/
void insertarEnOrden(struct Lista & tad, struct Elemento elemento) {
    ...
    tad.longitud++;
    tad.cola = obtenerUltimoNodo(tad);
}
...
struct Nodo * obtenerNodoAnterior(const struct Lista & tad, struct Elemento elemento) {
    ...
    struct Nodo * recorrido = tad.cabeza;
    while ((recorrido != nullptr) and (recorrido->elemento.impacto < elemento.impacto)) {
        anterior = recorrido;
        recorrido = recorrido->siguiente;
    }
    return anterior;
...
struct Nodo * seEncuentra(const struct Lista & tad, struct Elemento elemento) {
    struct Nodo * recorrido = tad.cabeza;
...
void eliminaNodo(struct Lista & tad, struct Elemento elemento) {
    struct Nodo * ultimo = nullptr;
    struct Nodo * recorrido = tad.cabeza;
...
void imprime(const struct Lista & tad) {
    ...
    if (esListaVacia(tad)) {
        ...
        while (recorrido != nullptr) {
            ...
            cout << recorrido->elemento.categoria<<" "<<recorrido->elemento.impacto;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
