...
void insertaPrioridad(struct Lista &lista, const struct Elemento & elemento){
    
    struct Nodo *nuevoNodo = crearNodo(elemento, nullptr); 
    
    if(elemento.tipo == 0){ //Es cliente
        if(lista.cola != nullptr){
            lista.cola->siguiente = nuevoNodo;
            lista.cola = nuevoNodo;
        }
        else if(lista.colaVip != nullptr){
