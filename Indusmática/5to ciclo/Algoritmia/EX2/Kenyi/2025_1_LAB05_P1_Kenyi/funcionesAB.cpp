...
void imprimirNodo(struct NodoArbol * raiz) {
    cout << setw(5) << raiz->elemento.categoria<<endl;
    if (strcmp(raiz->elemento.nombre, " "))cout << raiz->elemento.nombre << endl;
}

void imprimirNodo2(struct NodoArbol * raiz){
   cout << raiz->elemento.nombre; 
}
