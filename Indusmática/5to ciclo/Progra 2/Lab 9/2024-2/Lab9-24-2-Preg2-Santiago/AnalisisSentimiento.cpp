...
void AnalisisSentimiento::imprimir_analisis(){
    ...
    for(auto &x : comentarios){
        ...
        arch << x.first << right <<endl;
        arch << "Positivos: " << positivos << endl;
        arch << "Neutros: " << neutros << endl;
        arch << "Negativos: " << negativos << endl;
        
        arch << "Mayor polaridad: ";
        x.second.front().imprimir(arch);
        arch <<  x.second.front().GetPolaridad_total() << endl;
        arch <<"Menor polaridad: ";
        x.second.back().imprimir(arch);
        arch <<  x.second.back().GetPolaridad_total() << endl;
        imprimirNPrimeros(arch, 5);
        imprimirLinea(arch, '=');
    }
...
void AnalisisSentimiento::imprimirNPrimeros(ofstream &arch, int n){
    int cont = 0;
    bool sale = false;
    for(auto &x : comentarios){
        for(auto &oraciones : x.second){
            if(oraciones.GetPolaridad_total() == 0){
                arch << setw(3) << cont + 1 << ") ";
                oraciones.imprimir(arch);
                arch << setw(4)<<oraciones.GetPolaridad_total() << endl;
                cont++;
            }
            if(cont == n){
                sale = true;
...
void AnalisisSentimiento::imprimirLinea(ofstream &arch, char car){
